...
`scenarios.ts` must encode these rules literally as `assertionRules`; the evaluator accepts a `pass`/`fail` assertion only from one of its declared sources. `runtime_trace` means the benchmark-owned Runtime Mode native/action trace, not a production audit change. When no permitted authoritative source exists, emit the `unavailable` assertion variant with a closed reason and **no fabricated source/evidence digest**; it never falls back to a weaker source.
