...
> **Current branch-local handoff (2026-09-17):** `fix/fs-stat-not-found-20260917`, native-managed isolated worktree `a76d53ed-a229-4479-809c-e9137cddd9b0`. This block supersedes stale in-flight statements below; Git remains authoritative.
> Created from the clean, superseded `fix/git-diff-check-20260917` worktree because `main` contains pre-existing untracked `.freebuff/`, then merged current `main@ec7ea5f` into this branch. Do not modify/remove the source worktree or `.freebuff/`.
> Root cause: `FileSystemService.stat` let `lstat` ENOENT escape as a plain Error, so `errorPayload` reported INTERNAL_ERROR. The approved narrow fix maps only ENOENT to a typed NOT_FOUND after PathPolicy resolution; other OS errors and permission checks remain unchanged.
> RED evidence: `tests/fs-service.test.ts` 1 failed/6 passed, actual ENOENT versus expected NOT_FOUND, exit 1. GREEN: focused filesystem and MCP tests 20/20, exit 0. Full `npm run check`: TypeScript build and 754 passed/2 skipped, exit 0, after the first handoff edit. Native macOS suite: 215/215 passed, exit 0. Reverify after any subsequent edit or commit.
> Browser PAGE_NOT_FOUND/UNAVAILABLE was intermittent and did not reproduce on the subsequent four-tab smoke. Computer type_text batches up to 20 UTF-16 units by existing design; Calculator displayed only the first of two digits. Neither has a confirmed cross-application root cause or a code change in this branch.
> No running service, tunnel, Chrome profile, native code, remote branch or `main` was modified. Live runtime still predates this fix; local tests do not prove deployment.
> **Next exact step:** review final diff and whitespace checks, commit the verified fix locally on this branch, then record exact HEAD and post-commit checks. Push, PR, merge and runtime rollout require separate authorization and freshness-bound verification; investigate Browser/Computer only with reproducible root-cause evidence.
