...
> **Computer fixture investigation (2026-09-17):** Browser readiness fix is already committed locally as `2e62a80`, branch clean. Read-only `computer_health` reports input/accessibility permission available, but `computer_observe` found the user's ordinary Google Chrome/ChatGPT window frontmost, so no physical focus change, typing, application launch or Calculator comparison was attempted. Existing Swift `KeyboardActionTests` 12/12 pass. The separate fixture executable builds successfully in debug and release, and its owned `.build/staged/ChatGPTSystemComputerRuntimeFixture.app` was packaged and ad-hoc signed without launching. A non-posted CoreGraphics keyboard event carrying `45` retained both UTF-16 units on round-trip (`recovered=45`); this proves encoding into an event, not delivery/acceptance by a real app. The previous Calculator symptom remains unclassified. No native input production code was changed.
> **Next exact step:** obtain an explicitly quiet, user-approved moment to briefly bring the dedicated fixture app to front; type `45` as one `computer_type_text` call and as two separate single-character calls, observe its own AX status/text after each and optionally compare Calculator only while safe. Return focus to prior app only if verified safe. If RED reproduces in fixture, add a failing native test and minimal source fix; otherwise attribute the distinction carefully and do not change production code speculatively. Full `npm run check` previously platform-blocked; never reroute around it. Push/PR/merge/deploy and live runtime rollout require separate authorization.
