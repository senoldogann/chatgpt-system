...
> **Next exact step:** isolate potential intermittent `Command+A` selection/shortcut failure using dedicated fixture AX test and compare Calculator keyboard-versus-Unicode handling only if necessary. Require reproducible RED root cause before native change. Never speculatively alter Unicode chunking, bypass previously blocked full `npm run check`, or push/PR/merge/deploy/live update without separate authorization.
