...
> **Next exact step:** create a narrowly bounded, test-first target-readiness strategy for dynamic pages while retaining uniqueness, credential refusal, fail-closed page IDs, and timeout limits; verify with deterministic delayed-target and duplicate-target RED cases. Do not use `force` as a blanket speed fix. The separate Computer GUI fixture A/B still requires a quiet user-approved test window. After code changes rerun full Node/native gates and commit locally; push/PR/merge/deploy and live runtime rollout require separate authorization.
