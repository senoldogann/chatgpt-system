...
function assertSafeBundlePath(bundlePath) {
  ...
  if (resolved === path.parse(resolved).root || path.extname(resolved) !== ".app") {
    throw new Error("Unsupported output path. Computer runtime bundle output must be a non-root .app path.");
  }
}

export function buildComputerRuntimeBundlePlan(context = {}) {
...
<dict>
  ...
  <string>ChatGPT System uses screen capture only when locally enabled to let the approved computer runtime observe the Mac.</string>
</dict>
</plist>
`;
}
...
async function assertReleaseExecutable(sourcePath) {
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      throw new Error("Release computer runtime executable is missing. Run npm run build:computer:macos first.");
    }
    throw error;
  }

  if (!info.isFile() || info.isSymbolicLink()) {
    throw new Error("Release computer runtime executable must be a regular file.");
  }
}
...
export async function stageComputerRuntimeBundle(plan = buildComputerRuntimeBundlePlan()) {
...
function printHelp() {
  console.log(`Usage: node scripts/package-macos-computer-runtime.mjs [options]
...
async function main() {
  ...
  const plan = buildComputerRuntimeBundlePlan({
    ...(args.outputPath === undefined ? {} : { outputPath: args.outputPath }),
  });
  await stageComputerRuntimeBundle(plan);
...
if (process.argv[1] && path.resolve(process.argv[1]) === scriptPath) {
  main().catch((error) => {
    console.error(`[chatgpt-system] macOS computer runtime packaging failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 1;
  });
}
