...
export const SERVER_INSTRUCTIONS = [
  "chatgpt-system is the owner's personal development bridge: it runs on the owner's own computer and acts only on the projects and capabilities the owner configured, for the owner's own software development and automation.",
  "To continue a registered project, call project_resume with its exact alias (project_list when the alias is unknown) and reconcile Git state before any project mutation. If the project was moved or re-cloned, project_resume says so; call project_rebind as it describes. To work in a folder outside the configured roots, start a Project lease with session_authority_start (or project_resume) and pass its authorityLeaseId.",
  "Coding workflow, as in a desktop IDE agent: locate code with code_query (files for globs, search with regex/glob/contextLines, symbols, definition, references); read only what is needed with fs_read offset/limit, and batch independent reads with fs_read_many; change existing files with fs_edit (exact unique oldString) or fs_apply_patch_set for coordinated multi-file edits, and fs_write for new files; check with code_query diagnostics and project_check; review with git_file_review or git_diff before git_commit. Keep each change minimal and verify it before reporting it as done.",
  "Every tool call costs a full model step, so prefer fewer, larger calls: for a long-running command started with process_start, call process_logs with cursor and waitMs (up to 30000) instead of repeated quick status or log polls.",
  "If a long response is interrupted (for example 'Connection interrupted' or 'stream recovery polling timed out'), tool calls that already ran keep their effects: re-read the current state (git status, process_list, fs_read) and continue from it instead of repeating mutations.",
