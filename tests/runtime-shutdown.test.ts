...
describe("closeRuntimeResources", () => {
  it("stops computer JavaScript then computer, managed processes and browser before control and transport", async () => {
    ...
    await closeRuntimeResources({
      runtime: {
        ...
        terminalSessionSupervisor: { close: async () => { calls.push("terminal-sessions"); } },
    ...
    expect(calls).toEqual(["computer-js", "computer", "terminal-sessions", "owner-shell", "processes", "browser", "continuity", "control", "transport"]);
  });
  ...
  it("continues later cleanup phases after computer JavaScript, computer, process and browser failures", async () => {
    ...
    await closeRuntimeResources({
      runtime: {
        ...
        terminalSessionSupervisor: {
          close: async () => {
            calls.push("terminal-sessions");
            throw new Error("terminal session stop failed");
          },
        },
    ...
    expect(calls).toEqual(["computer-js", "computer", "terminal-sessions", "owner-shell", "processes", "browser", "continuity", "control", "transport"]);
    expect(errors).toEqual(["computer-js", "computer", "terminal-sessions", "owner-shell", "processes", "browser", "continuity"]);
  });
});
