...
describe("ScopedComputerService policy and audit", () => {
  ...
  it("audits bounded semantic scrolling without target or page content", async () => {
    const { auditFile, audit } = await fixture();
    const backend = new FakeComputerBackend();
    const service = new ScopedComputerService(backend, audit, true);

    await service.scrollUntilVisible({
      target: { by: "ocrText", text: "SCROLL_TARGET_PRIVATE_CANARY", exact: true },
      within: { by: "role", role: "AXScrollArea", name: "SCROLL_SCOPE_PRIVATE_CANARY", exact: true },
      direction: "down",
      amount: "page",
      maxSteps: 4,
    });
    ...
    expect(log).not.toContain("SCROLL_TARGET_PRIVATE_CANARY");
    expect(log).not.toContain("SCROLL_SCOPE_PRIVATE_CANARY");
  });
