...
15a. **Owner Runtime is separately explicit full-host execution**: `shell_run` is disabled unless the daemon starts with `--enable-owner-runtime` / `CHATGPT_SYSTEM_ENABLE_OWNER_RUNTIME=true`, requires the existing Admin profile and Personal Admin startup mode, and never becomes available to Project/User. It executes arbitrary login-shell syntax as the current OS user and is not an OS sandbox. The trusted shell executable is startup configuration rather than MCP input. Retained stdout/stderr are bounded tails; script/output/environment/lease content is not persisted to audit. Timeout, MCP cancellation, and daemon shutdown terminate the owned process group.
...
Personal Admin does **not** imply terminal, Owner Runtime, or browser capability. If the daemon was not also started with `--enable-terminal`, the resulting Admin lease has host-wide filesystem scope but `terminalEnabled=false` and an empty command allowlist. `shell_run` additionally requires the separate `--enable-owner-runtime` gate; enabling it does not change `terminal_run` and never widens Project/User. If browser startup is not enabled, browser content/action tools remain unavailable. These independent runtime gates remain authoritative.
