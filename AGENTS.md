...
If ChatGPT says `This conversation does not support developer MCPs`, do not repeatedly retry the unavailable namespace or substitute a container for the user's Mac. Continue in a new supported standard text chat in the same Project and call `project_resume` before mutation after capability returns. `Connection interrupted. Waiting for the complete answer` or `stream recovery polling timed out` alone does not establish local failure; do not restart a healthy tunnel; re-read state before repeating mutations. Use `npm run diagnose:chatgpt` near the incident time and distinguish historical evidence from present health.
